...
pipeline.onset = 0.2                 # 0.767 по умолчанию. Порог включения речи — насколько сильно модель должна "поверить", что начался голос. меньше значение → больше чувствительность
pipeline.offset = 0.4                # 0.377 по умолчанию. Порог выключения речи — насколько сильно модель должна "поверить", что речь закончилась. Меньше значение → чувствительнее к тишине, будет быстрее обрывать речь.
pipeline.min_duration_on = 0.2       # 0.136 по умолчанию. Минимальная продолжительность речи, чтобы она была учтена как отдельный фрагмент. Если голос прозвучал менее чем на 136 мс, он игнорируется полностью — считается шумом (например, шорох, вздох, "эм").
pipeline.min_duration_off = 0.1      # 0.067 по умолчанию Минимальная продолжительность паузы, чтобы считалась настоящей тишиной между спикерами. Если тишина короче 67 мс, она игнорируется и две реплики сливаются в одну.
