...
SIMILARITY_THRESHOLD = 0.75       # Порог косинусного сходства голосовых эмбеддингов (0.0-1.0)
                                   # - Выше значение = более строгое сравнение (только очень похожие голоса считаются одним спикером)
...
MAIN_CLUSTER_BIAS = 0.05           # Смещение в пользу определения основного кластера как "я" (0.01-0.1)
                                   # - Позволяет скорректировать определение главного спикера
                                   # - Увеличивает порог определения основного кластера на указанный процент
...
WHISPER_BEAM_SIZE = 5              # Качество распознавания речи (целое число, обычно 1-10)
                                   # - Больше значение = лучше качество, но значительно медленнее
                                   # - Меньше значение = быстрее обработка, но возможны ошибки
                                   # Для важных задач рекомендуется 5-7

WHISPER_LANGUAGE = "ru"            # Язык для распознавания речи (код языка ISO 639-1)
                                   # - "ru" - русский
                                   # - "en" - английский
                                   # - "auto" для автоматического определения
                                   # - Полный список: https://github.com/openai/whisper

SUPPRESS_EMPTY = True              # Фильтрация пустых сегментов (True/False)
                                   # - True: удалять сегменты без распознанного текста
                                   # - False: сохранять все сегменты, даже пустые

# === Параметры объединения сегментов ===
MERGE_MIN_DURATION = 0.5           # Минимальная длительность сегмента для объединения (секунды)
                                   # - Короткие сегменты короче этой длительности будут объединяться
                                   # с соседними сегментами того же спикера

MERGE_MAX_GAP = 0.5                # Максимальный временной разрыв между сегментами для объединения (в секундах)
                                   ...
                                   #   в один непрерывный сегмент в итоговом JSON
                                   #
                                   # Механизм работы:
                                   # 1. Скрипт сначала получает сегменты от диаризации (кто говорит) и транскрипции (что сказано)
                                   # 2. Эти сегменты могут не совпадать по временным границам
                                   # 3. MERGE_MAX_GAP определяет, при каком максимальном промежутке между сегментами
                                   #    они все еще считаются частью одной реплики и объединяются
                                   #
                                   # Примеры:
                                   # - Значение 0.4: паузы до 400 мс между словами считаются естественными
                                   #   и не разрывают реплику на отдельные сегменты
                                   # - Для быстрой речи можно уменьшить до 0.2-0.3
                                   # - Для медленной, размеренной речи можно увеличить до 0.5-0.6
                                   #
                                   # Влияние на результат:
                                   # - Большее значение: больше объединений, меньше сегментов в итоговом JSON
                                   # - Меньшее значение: меньше объединений, более точное соответствие
                                   #   оригинальным паузам в речи

# === Параметры цензуры ===
ALLOW_PROFANITY = True             # Разрешить ненормативную лексику (True/False)
                                   # - True: разрешает все слова без фильтрации
                                   # - False: включает встроенную цензуру Whisper
if ALLOW_PROFANITY:
    WHISPER_SUPPRESS_TOKENS = []   # Пустой список отключает цензуру
else:
    WHISPER_SUPPRESS_TOKENS = None # Стандартная цензура Whisper
